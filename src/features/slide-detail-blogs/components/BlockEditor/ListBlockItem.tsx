...
import type { ListBlock, ListType, ListStyle, ListItem } from "@/types/slide-detail-blog";
import {
  normalizeListItems,
  flattenListItems,
  indentListItem,
...
export function ListBlockItem({ block, onChange }: ListBlockItemProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {showStylePanel && (
        <div className="grid grid-cols-1 gap-3 rounded-lg border border-border bg-surface-muted/50 p-3 sm:grid-cols-3">
          <div>
            <label className="mb-1 block text-[11px] font-medium text-text-muted">
              Cỡ chữ ({block.fontSize ?? 15}px)
            </label>
            <input
              type="range"
              min={12}
          ...
          <div>
            <label className="mb-1 block text-[11px] font-medium text-text-muted">
              Độ cao dòng ({block.lineHeight ?? 1.75})
            </label>
            <input
              type="range"
              min={1.2}
          ...
          <div>
            <label className="mb-1 block text-[11px] font-medium text-text-muted">
              Khoảng cách mục ({block.itemSpacing ?? 6}px)
            </label>
            <input
              type="range"
              min={2}
              max={24}
              step={1}
              value={block.itemSpacing ?? 6}
              onChange={(e) => handleItemSpacingChange(parseInt(e.target.value, 10))}
              className="w-full cursor-pointer accent-primary"
            />
          </div>
        </div>
        ...
        return (
          <div className="space-y-2" style={containerStyle}>
            {flatItems.map((flat, index) => {
              const loc = findItemLocation(items, flat.item.id);
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-text-muted">
        ...
        <span>•</span>
        <span><kbd className="rounded border border-border bg-surface-muted px-1 py-0.5 font-mono text-[10px]">Tab</kbd> thụt cấp</span>
        <span>•</span>
