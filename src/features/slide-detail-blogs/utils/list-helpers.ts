...
export function resolveListLevelStyle(
  ...
  if (levelOverride?.marker) {
    ...
    else if (depth >= 2) marker = "upper-alpha";
    else marker = "decimal";
  } else {
    // Bullet default
...
export function flattenListItems(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    result.push({
      item,
      depth,
      canIndent: i > 0, // Item must have a preceding sibling at the same level to indent
      canOutdent: depth > 0, // Item must be at depth > 0 to outdent
      parent,
    });
