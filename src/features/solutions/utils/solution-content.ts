...
export function htmlToDocumentBlocks(html: string): ContentBlock[] {
  ...
  // If plain text without HTML tags (e.g. current 19 solution records in DB)
...
/**
 * Parses raw solution content from DB into DocumentContent.
 * Handles:
 * 1. Structured DocumentContent object (modern backend JSONB)
 * 2. Serialized JSON string
 * 3. Legacy text/HTML strings (19 existing database solutions)
 */
export function parseSolutionContent(rawContent: unknown): DocumentContent {
  if (!rawContent) {
  ...
  if (typeof rawContent === "string") {
    ...
    if (trimmed.startsWith("{")) {
      try {
        ...
        // Not JSON, continue to text/HTML converter
      }
    }

    // Convert legacy text or HTML to DocumentContent
    return {
      version: 1,
...
/**
 * Serializes DocumentContent into JSON string for debug or storage.
 */
export function serializeSolutionContent(doc: DocumentContent): string {
