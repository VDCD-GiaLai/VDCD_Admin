 ...
 * Reads from the same ContentBlock[] data structure used by the editor.
 */
export function DocumentContentRenderer(props: DocumentContentRendererProps) {
  const blocks = props.blocks ?? props.content?.blocks ?? [];
  if (blocks.length === 0) {
    return (
      ...
      {blocks.map((block: ContentBlock) => {
        ...
        const renderBlock = () => {
          switch (block.type) {
            ...
            case "ordered_list":
              return <ListBlockRenderer block={{ ...block, listType: "ordered" } as unknown as ListBlock} />;
            case "quote": {
