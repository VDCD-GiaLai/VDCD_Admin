...
import {
  ...
  generateId,
} from "../model/block-factory";

function cloneBlockWithNewIds(original: ContentBlock): ContentBlock {
  const cloned: ContentBlock = JSON.parse(JSON.stringify(original));
...
export function BlockEditor({ value, onChange }: BlockEditorProps) {
  ...
  const handleSelectBlockType = (type: ContentBlock["type"]) => {
    let newBlock: ContentBlock;

    switch (type) {
      case "heading":
        newBlock = createHeadingBlock({ level: 2, text: "" });
        break;
      case "paragraph":
        newBlock = createParagraphBlock({ text: "" });
        break;
      case "image":
        newBlock = createImageBlock({ url: "", alt: "", caption: null });
        break;
      case "list":
        newBlock = createListBlock();
        break;
      case "section":
        newBlock = createSectionBlock({
      ...
      case "cta":
        ...
        break;
      case "quote":
        newBlock = createQuoteBlock({ text: "" });
        break;
      case "highlight":
        newBlock = createHighlightBlock({ text: "" });
        break;
      case "ordered_list":
        newBlock = createListBlock({ listType: "ordered" });
        break;
      default:
        newBlock = createParagraphBlock({ text: "" });
        break;
