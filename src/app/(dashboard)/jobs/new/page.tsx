...
export default function CreateJobPage() {
  ...
  const onSubmit = (data: JobFormData) => {
    ...
    createMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: "Tạo vị trí tuyển dụng thành công", color: "success" });
          router.push("/jobs");
        },
        onError: (error) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-text">Thêm vị trí tuyển dụng</h1>
          <p className="text-sm text-text-muted">Tạo mới thông tin tuyển dụng.</p>
        </div>
