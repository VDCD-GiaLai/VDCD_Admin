...
export default function EditSlideDetailBlogPage() {
  ...
  const handleHeroFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Track ALL replaced hero fileIds (including original DB hero) for cleanup at save time
      if (previousUnsavedFileId) {
        setDiscardedHeroFileIds((prev) =>
          prev.includes(previousUnsavedFileId) ? prev : [...prev, previousUnsavedFileId],
        );
      }
  ...
  // Backend no longer auto-deletes hero images during PATCH — frontend manages
  // the full lifecycle by tracking ALL discarded fileIds and deleting in onSuccess.
  const handleDeleteHeroImage = () => {
    const currentFileId = getValues("heroImageFileId");
    // Track ANY fileId being removed (including the original DB hero) for cleanup at save time
    if (currentFileId) {
      setDiscardedHeroFileIds((prev) =>
    ...
    toast({
      title: "Đã gỡ ảnh bìa",
      description: "Nhấn \"Lưu thay đổi\" để hoàn tất gỡ bỏ trên hệ thống và ImageKit.",
      color: "warning",
    });
  };

  // Handle Soft-Delete of Image Blocks
  // When a content image block is removed or its image is replaced, track the fileId
  // so we can clean up ImageKit on a successful save.
