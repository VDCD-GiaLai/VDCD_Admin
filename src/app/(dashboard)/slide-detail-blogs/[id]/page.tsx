...
export default function EditSlideDetailBlogPage() {
  ...
  const handleGallerySelect = (image: ImagePickerResult) => {
    ...
    if (previousFileId) {
      if (!galleryFileIds.includes(previousFileId)) {
        if (previousFileId !== blog?.heroImageFileId) {
  ...
  // Handle Delete Hero Image (soft-delete for gallery images, hard-delete for direct session uploads)
  const handleDeleteHeroImage = () => {
    ...
    const isDirectSessionUpload =
      ...
      currentFileId !== blog?.heroImageFileId;

    if (currentFileId && !galleryFileIds.includes(currentFileId)) {
      if (currentFileId !== blog?.heroImageFileId) {
      ...
      // DB-persisted hero image: preserved in ImageKit (soft-delete from blog only)
    }
          ...
          if (discardedFileIds.length > 0) {
            for (const fid of discardedFileIds) {
              if (
                ...
                fid !== updatePayload.heroImageFileId
              ) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`inline-flex items-center rounded-md px-2.5 py-0.5 text-xs font-semibold ${blog.isPublished
            ...
            onHeroImageChange={(url, fileId) => {
              ...
              if (prevFileId && prevFileId !== fileId) {
                if (!galleryFileIds.includes(prevFileId)) {
                  if (prevFileId !== blog?.heroImageFileId) {
